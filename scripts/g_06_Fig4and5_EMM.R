...
plot_LRCurve_perExposure_2Mods <- function(sensitivityA, subSetVarA, subSetA, ERConstraintA, LRConstraintA,
  ...
  # sensitivityA <- 'Main' ; subSetVarA <- 'ice'; subSetA <- 'iceLow'
  # ERConstraintA <- 'selectedMain'; LRConstraintA <- 'selectedMain'
  # sensitivityB <- 'Main' ; subSetVarB <- 'ice'; subSetB <- 'iceHigh'
...
png(here::here(
  outPath, "manuscript",
  paste0("Fig4_compareLagR_", "catchmentArea", ".png")
),
...
png(here::here(
  outPath, "manuscript",
  paste0("Fig5_compareLagR_", "ice", ".png")
),
width = ww.fig * 2, height = hh.fig * 0.75, res = rr.fig * 1
)
