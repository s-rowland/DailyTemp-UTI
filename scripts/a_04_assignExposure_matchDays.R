...
cases <- cases %>% 
  rename(case_count_ice_iceLow = case_count_low_ice) %>% 
  mutate(case_count_ice_iceHigh = case_count - case_count_ice_iceLow, 
...
if (outcomeName == 'UTI') {
  ...
  cases.kp <- cases %>% 
    filter(catchmentArea == 'kpsc') %>% 
    mutate(adate = parse_date_time(adate, 'mdy)'))
  # 1c.ii Clean up Sutter's date variable
  cases.sut <- cases %>% 
    filter(catchmentArea == 'sutter') %>% 
    mutate(adateNum = as.numeric(adate)) %>%
    mutate(adate = parse_date_time('01/01/1970', 'mdy') + 24*60*60*adateNum) %>% 
    dplyr::select(-adateNum)
  # 1c.iii Combine
  cases <- bind_rows(cases.kp, cases.sut)
  rm(cases.kp, cases.sut)
